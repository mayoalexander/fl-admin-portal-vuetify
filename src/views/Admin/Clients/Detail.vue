<template>
  <div class="px-4 white--text">
    <v-layout wrap>
      <v-flex xs12 class="px-2 mb-3 text-xs-center">
        <div class="data-box pa-4">
          <!-- Email & Phone + Dashboard Type -->
          <p v-if="user.email" class="title mb-2">{{ user.email }}</p>
    ...
    <v-layout wrap>
      <v-flex xs12 sm6 class="mb-3 text-xs-center px-2">
        <div class="data-box pa-4">
    ...
    <v-layout wrap>
      <v-flex xs12 sm6 class="mb-3 text-xs-center px-2">
        <div class="data-box pa-4">
          <!-- Profiles -->
          <div v-if="user.profiles" class="profiles-list">
            <li v-for="item in user.profiles" :key="item.pk">
              {{ item }}
            </li>
          </div>
          <div v-else>
      ...
      <v-flex xs12 sm6 class="mb-3 text-xs-center px-2">
        <div class="data-box pa-4">
          Activity
          <div v-if="user.activity" class="activity-list">
            <li v-for="item in user.activity" :key="item.pk" class="mb-3">
    ...
    <v-layout wrap>
      ...
      <v-flex xs12 sm6 class="mb-3 text-xs-center px-2">
        <div class="data-box pa-4">
          Notifications
          <div v-if="user.notifications" class="notifications-list my-3 mt-4">
            <div v-for="item in user.notifications" class="mb-3 b-b pb-3">
